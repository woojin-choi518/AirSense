    ...
    >
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="md:hidden">
          <TextContent title={title} subTitle={subTitle} description={description} isDark={isDark} isMobile={true} />

          <ViewToggleButton view={view} onViewChange={setView} isDark={isDark} />

          <ImageDisplay view={view} image1={image1} image2={image2} isMobile={true} />

          <p className={`text-center ${isDark ? 'text-[#F5E6C8]' : 'text-gray-900'}`}>{description}</p>
        </div>

        {/* 데스크탑 전용 그리드 */}
        <div
          className="hidden gap-x-4 gap-y-70 md:grid"
        ...
        >
          ...
          <motion.div style={{ x: row1X, opacity: row1Opacity }}>
            <ImageDisplay view="first" image1={image1} image2={image2} isMobile={false} />
          </motion.div>

          {/* (2,1) 작은 캡처 - 오른쪽에서 등장 (row2) */}
          <motion.div style={{ x: row2X, opacity: row2Opacity }}>
            <ImageDisplay view="second" image1={image1} image2={image2} isMobile={false} />
          </motion.div>
